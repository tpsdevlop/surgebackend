...
def getRankings(COURSE,SID):
    try:
        userRank = get_cache ("Rankings.objects.filter(Course = '"+COURSE+"',StudentId = '"+SID+"').first()")
        if userRank is  None:
            userRank = Rankings.objects.filter(Course = COURSE,StudentId = SID).first()
            create_cache("Rankings.objects.filter(Course = '"+COURSE+"',StudentId = '"+SID+"').first()",userRank,60)
...
def  OverallRankings(COURSEs,SID):#COURSEs = ["HTMLCSS", "Java_Script"] , SID = "24ADMI0001"
    try:
        aggregate_scores = get_cache("Rankings.objects.filter(Course__in=COURSEs).values('StudentId').annotate(total_score=Sum('Score')).order_by('-total_score')")
        if aggregate_scores is None:
            ...
            create_cache("Rankings.objects.filter(Course__in=COURSEs).values('StudentId').annotate(total_score=Sum('Score')).order_by('-total_score')", aggregate_scores, 60 )
        rank = 1
